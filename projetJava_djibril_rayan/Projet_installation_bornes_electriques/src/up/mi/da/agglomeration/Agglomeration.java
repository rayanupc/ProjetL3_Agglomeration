...
public class Agglomeration {
	...
	public String villeVoisineSansBorne(Ville v) {
		int k = v.getId();
		boolean villeVoisineAvecAucyneBorne = true;
		StringBuilder sb = new StringBuilder("");
		for(int i=0; i<this.enssembleDeVille.size(); i++) {
			if(this.graphe.getMatrice()[k][i]==true) {
				/*
				 * On verifie si les villes voisine de v possede une borne, 
				 * et si elle ne possede pas borne, on verifie si les ville voisines des villes voisine de v 
				 * possedent une borne
				 */
				if(this.rechercheVille(i).getPossedeUneBorne()==false) {
					for(int j=0; j<this.enssembleDeVille.size(); j++) {
						//Si on tombe sur l'id de la ville v on passe car on n'a pas besoin de savoir 
						//si v à une borne ou pas.
						if(j==k) {
							continue;
						}
						else {
							if(this.graphe.getMatrice()[i][j]==true) {
								if(this.rechercheVille(j).getPossedeUneBorne()==true) {
									villeVoisineAvecAucyneBorne = false;
								}
							}
						}
					}
					if(villeVoisineAvecAucyneBorne==true) {
						sb.append(this.rechercheVille(i).getNom()+", ");
					}
				}
			}
		}
	...
	/**
	 * Permet d'indiquer s'il est encore possible de retire une zone de recharge
	 * 
	 * @return true s'il est possible de retirer une zone de recharge, false sinon
	 */
	public boolean possibleDeRetirerUneBorne() {
		Graphe grapheAuxiliaire = new Graphe();
		Agglomeration aggloAuxiliaire = new Agglomeration(grapheAuxiliaire);
		aggloAuxiliaire.setGraphe(this.getGraphe());
		aggloAuxiliaire.setEnssembleDeVille(this.getEnssembleDeVille());
		int nbVilleAvecBorne = 0;
		for(Ville v : aggloAuxiliaire.getEnssembleDeVille()) {
			if(v.getPossedeUneBorne()==true) {
				nbVilleAvecBorne +=1;
			}
		}
		int compteurDeNonRespectAccessibilite = 0;
		for(Ville v : aggloAuxiliaire.getEnssembleDeVille()) {
			if(v.getPossedeUneBorne()==true) {
				v.setPossedeUneBorne(false);
				if(aggloAuxiliaire.respecteConditionAccessibilite()==false) {
					compteurDeNonRespectAccessibilite+=1;
				}
				v.setPossedeUneBorne(true);
			}
		}
		if(nbVilleAvecBorne==compteurDeNonRespectAccessibilite) {
			return false;
		}
		else {
			return true;
		...
		do {
			try {
				...
				case 2:
						...
						if(this.possibleDeRetirerUneBorne()==false) {
							System.err.println("ATTENTION, DORENAVANT VOUS NE POUVEZ PLUS RETIREZ DE ZONE DE RECHARGE. \n"
									+"Vous êtes peut etre arrivé à une solution du problème ou même à une solution optimale !!!");
						}
					break;
				case 3:
	 ...
	 */
	public void menuSauvegardeAgglomeration(Scanner sc) {
		int choixSave = 0;
		do {
			try {
			...
			case 2:
		        ...
		        if (!file.exists()) {
		            System.err.println("Le fichier spécifié n'a pas été trouvé : " + fichierSave);
		        }
		        else {
					SauvegardeAgglomeration.sauvegardeAgglomeration(this, fichierSave);
		        }
